const artistTable: string = `CREATE TABLE IF NOT EXISTS artists (
...
const getArtists: string = `SELECT * FROM artists
WHERE extract(DAY FROM age(date)) <= $1 AND extract(MONTH FROM age(date)) = 0;`

const incrLikes: string = 'UPDATE artists SET likes = likes + 1 WHERE id = $1;';

const getLeaderBoard: string = `SELECT * FROM artists
